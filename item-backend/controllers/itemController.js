...
const createUserItem = async (req, res) => {
  try {
    ...
    if (!name || !category || !selected_unit) {
      return res
        .status(400)
        .json({ message: "Name, category, and selected_unit are required." });
    }
    ...
    if (existingItem) {
      ...
      const unitExists = existingItem.packaging.some(
        (pack) => pack.unit === selected_unit
      );
      ...
      if (unitExists) {
        return res
          .status(400)
          .json({
            message: `Unit '${selected_unit}' already exists for '${name}'.`,
          });
      }
    ...
    return res
      .status(201)
      .json({ message: "User item created successfully.", item: newItem });
  } catch (error) {
...
const updateUserItem = async (req, res) => {
  try {
    ...
    if (!existingItem) {
      return res
        .status(404)
        .json({ message: `Item '${name}' not found for this user.` });
    }
    ...
    if (packaging && Array.isArray(packaging)) {
      packaging.forEach((newPackage) => {
        const existingPackageIndex = existingItem.packaging.findIndex(
          (pack) => pack.unit === newPackage.unit
        );

        if (existingPackageIndex !== -1) {
          // Update `times_bought` and `times_wasted` if provided
          if (newPackage.times_bought !== undefined) {
            existingItem.packaging[existingPackageIndex].times_bought =
              newPackage.times_bought;
          }
          if (newPackage.times_wasted !== undefined) {
            existingItem.packaging[existingPackageIndex].times_wasted =
              newPackage.times_wasted;
          }
        } else {
...
const addOrUpdateUserItem = async (req, res) => {
  try {
    ...
    if (!existingItem) {
      ...
      const newItem = new Item({
        ...
        packaging: [
          {
            unit: selected_unit,
            times_bought: 0, // Start with 1 purchase
            times_wasted: 0,
          },
        ],
      });
    ...
    // Check if the selected unit already exists in the packaging array
    const existingPackaging = existingItem.packaging.find(
      (pack) => pack.unit === selected_unit
    );
