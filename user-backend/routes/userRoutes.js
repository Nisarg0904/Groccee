...
router.post("/send-verification-email", limiter, async (req, res) => {
  ...
  try {
    ...
    user.verificationTokenExpiry = new Date(Date.now() + 60 * 60 * 1000); // 1 hour expiry
    await user.save();
...
router.post("/send-password-reset", limiter, async (req, res) => {
  const { email } = req.body;
...
router.post("/resend-verification-email", limiter, async (req, res) => {
  const { email } = req.body;
